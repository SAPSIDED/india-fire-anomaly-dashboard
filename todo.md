...
- [x] Deploy FireGuard to the user's Vercel account from the connected GitHub repository, including safe frontend/backend routing, environment configuration, and live smoke testing.
...
- [x] Push the validated configuration to `main`, redeploy Vercel, and verify the production homepage, hotspot query, and marker-verification flow.
- [x] Restore and validate the approved FRP-null-safe, leakage-safe two-class training dataset after project-state recovery.
...
- [x] Restore the richer Vercel map experience: Google-style satellite imagery, map/satellite controls, supported street-view/Pegman interaction, live marker popups, and button-driven overlay-state changes, without changing FIRMS or classification backend behavior. Vercel now has the enhanced fallback controls, explorer, popups, and overlay states; Google-only imagery/Pegman remains blocked by the external Manus Maps proxy HTTP 401.
- [x] Improve the Vercel map fallback to provide richer satellite/map controls, hotspot/place popups, an animated draggable location character where feasible, and visible button-driven UI state changes while preserving live verification callbacks. Exact Google-only satellite/Pegman remains blocked by the Manus Maps proxy HTTP 401 for the Vercel origin.
- [x] Regenerate the leakage-safe two-class dataset from the current live database and report industrial, wildfire, mining, and agricultural-burning eligibility.
- [x] Run the existing group-aware XGBoost and logistic-regression 5-fold evaluation on the fresh dataset and save the requested model/report artifacts without deployment.
- [x] Compare fresh metrics with the previous 70.2% ±9.3% XGBoost benchmark and deliver exact results and limitations.
...
- [x] Add an isolated ML comparison signal using the current trained XGBoost model and approved four-feature contract; keep the rule-based classifier authoritative and unchanged.
- [x] Expose additive ML prediction/comparison data through a Vercel-compatible backend path with safe unavailable/error states and no live inference deployment until validated.
- [x] Add the Rule vs ML comparison UI to the dashboard, test it, and deploy it through the connected Vercel project without changing FIRMS retrieval, scheduler, or database schema.
...
- [x] Inspect the repository copy of the Anaconda-trained XGBoost artifact, approved feature order, model labels, and current Vercel deployment contract.
- [x] Implement a lightweight Node.js XGBoost JSON inference adapter that matches the trained four-feature preprocessing and returns an ML comparison signal without replacing rule classification.
- [x] Add the ML comparison result additively to the existing corroboration response and preserve all existing response fields and retrieval behavior.
